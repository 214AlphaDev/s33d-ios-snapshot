...
class InventoryItemEditScreen: UIViewController, KeyboardDismissable, UITextViewDelegate, ActivityViewProvider, InventoryItemEditViewProtocol {
    
    var presenter: WeakWrapper<InventoryItemEditPresenterProtocol> = WeakWrapper()
    ...
    var category: CategoryPicker.Category = .book {
        didSet {
            categoryPicker?.setCategory(category)
        }
    }
    ...
    func createNavigationBar(in container: UIView) -> UINavigationBar {
        ...
        let submitItem = UIBarButtonItem(title: "Submit", style: .plain, target: self, action: #selector(submitTapped))
    ...
    func setInventoryItem(_ item: InventoryItem) {
        ...
        category = CategoryPicker.Category(inventoryCategory: item.category)
        nameTextField.text = item.name
        descriptionTextView.text = item.description
        storyTextView.text = item.story
    ...
    func updateSubmitEnabled() {
        guard submitBarButtonItem != nil else {
            return
        }
...
extension InventoryItemEditScreen: ImagePickerControllerDelegate {
