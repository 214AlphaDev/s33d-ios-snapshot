...
class ChatViewController: UIViewController,UITextViewDelegate,UITableViewDelegate, UITableViewDataSource, UIDocumentMenuDelegate,UIDocumentPickerDelegate,UINavigationControllerDelegate,AVAudioPlayerDelegate,AVAudioRecorderDelegate,UIGestureRecognizerDelegate,QLPreviewControllerDataSource,QLPreviewControllerDelegate, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        if(isGroup == "1"){
            messageRequest = MessagesRequest.MessageRequestBuilder().set(guid: buddyUID).hideMessagesFromBlockedUsers(true).set(limit: 10).build()
        }else{
            messageRequest = MessagesRequest.MessageRequestBuilder().set(uid: buddyUID).set(limit: 10).build()
        }
    ...
    func scrollToBottom() {
        if self.chatMessage.count > 1 {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.214) {
    ...
    func handleOneOnOneChatVCApperance(){
        ...
        // The hiding is done by commenting out the image and color below
        // The reason being UIBarButtonItem doesn't have a .isHidden property
        ...
        // Disable button instead of removing it to maintain proper UI
        // Can be re-enabled once additional features are added
        // Currently it's related to chat message search functionality
...
/////////////////////////////////// Setting up action sheet //////////////////////////////////
        ...
        case .text where messageData.sender?.uid == myUID:
            ...
            // The break below disables the edit and delete message functionality
            // This action triggeres when a message is clicked
            break
