...
class OneOnOneListViewController: UIViewController {
    ...
    func  handleContactListVCAppearance(){
        ...
        navigationItem.title = NSLocalizedString(Contants.chatTabBarTitle, comment: "")
        ...
        // Disable button instead of removing it to maintain proper UI
        // Can be re-enabled once additional features are added
        moreButton.isEnabled = false
            
        // The button can be hidden here as it doesn't negatively influence the UI
        ...
        if(UIAppearanceColor.SEARCH_BAR_STYLE_LIGHT_CONTENT == true){
            UITextField.appearance(whenContainedInInstancesOf: [UISearchBar.self]).attributedPlaceholder = NSAttributedString(string: NSLocalizedString(Contants.chatSearchTitle, comment: ""), attributes: [NSAttributedString.Key.foregroundColor: UIColor.init(white: 1, alpha: 0.5)])
        }else{
            UITextField.appearance(whenContainedInInstancesOf: [UISearchBar.self]).attributedPlaceholder = NSAttributedString(string: NSLocalizedString(Contants.chatSearchTitle, comment: ""), attributes: [NSAttributedString.Key.foregroundColor: UIColor.init(white: 0, alpha: 0.5)])
        }
        
        UITextField.appearance(whenContainedInInstancesOf: [UISearchBar.self]).defaultTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor(hexFromString: Contants.signupScreenTextColor)]
        ...
        if let textfield = searchController.searchBar.value(forKey: "searchField") as? UITextField {
            textfield.textColor = UIColor(hexFromString: Contants.signupScreenTextColor)
...
extension OneOnOneListViewController : UITableViewDelegate, UITableViewDataSource {
   ...
   //numberOfRowsInSection:
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        //Replace the line below with the commented one above to enable blockAction on swiping
        //Make sure that the rest of the functionaltiy necessary is in place before enabling the UI element
    ...
    func tableView(_ tableView: UITableView, leadingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        if AMShimmer.isAnimating == false{
            ...
            //Replace the line below with the commented one above to videoCall,audioCall on swiping
            //Make sure that the rest of the functionaltiy necessary is in place before enabling the UI element
...
extension OneOnOneListViewController : CometChatMessageDelegate {
    ...
    func onTypingStarted(_ typingDetails: TypingIndicator) {
        // Disable typing indicator instead of removing it to maintain proper UI
        // Can be re-enabled once additional features are added by deleting the line below
        return
