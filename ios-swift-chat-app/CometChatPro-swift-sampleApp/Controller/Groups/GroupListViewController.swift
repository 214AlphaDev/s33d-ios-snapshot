...
class GroupListViewController: UIViewController , UITableViewDelegate , UITableViewDataSource , UISearchBarDelegate {
            ...
            cell.getUnreadCountForAllGroups(UID: group.guid) { (count, error) in
                ...
                DispatchQueue.main.async {
                    if count == 0 {
                        ...
                        // Disable unreadCountBadge instead of removing it to maintain proper UI
                        cell.unreadCountBadge.isHidden = true
                        
                        // Can be re-enabled once additional features are added by uncommenting the line below
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        // Remove the line below to enable group deleteAction,leaveAction when functionality is implemented
        confrigation = UISwipeActionsConfiguration(actions: [])
    ...
    func tableView(_ tableView: UITableView, leadingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        // Remove the line below to enable group videoCall,audioCall when functionality is implemented
        confrigation = UISwipeActionsConfiguration(actions: [])
